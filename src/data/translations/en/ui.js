// ============================================================================
// ENGLISH TRANSLATIONS - UI
// ============================================================================

export const uiTranslations = {
  meta: {
    language: 'en',
    languageName: 'English',
    version: '1.0'
  },
  
  app: {
    title: "UNITYQUEST Chronicles\nof Love",
    subtitle: "Interactive Scripture Discovery",
    tagline: "Walking toward divine light",
    menu: "Menu",
    retry: "Retry"
  },
  
  buttons: {
    play: "PLAY",
    back: "Back",
    continue: "Continue",
    restart: "Restart",
    close: "Close",
    nextLevel: "Next Level",
    selectLevel: "Level Selection",
    retry: "Retry",
    mainMenu: "Main Menu",
    start: "Start",
    cancel: "Cancel",
    confirm: "Confirm",
    save: "Save",
    load: "Load",
    yesRestart: "Yes, restart",
    noContinue: "No, continue"
  },
  
  labels: {
    score: "Score",
    stars: "Stars",
    wisdom: "Wisdom",
    revelation: "Revelation",
    lives: "Lives",
    combo: "Combo",
    level: "LEVEL",
    easy: "EASY",
    medium: "MEDIUM",
    hard: "HARD",
    correct: "Correct!",
    gameOver: "Trial Failed",
    victory: "Victory!",
    completed: "Completed",
    rank: "Spiritual Rank",
    points: "Wisdom Points",
    unlockedLevels: "level(s) unlocked",
    questionOfKnowledge: "Question of Knowledge",
    hintOfWisdom: "Hint of Wisdom",
    knowledgeMastered: "Knowledge Mastered!",
    knowledgeAcquired: "Knowledge acquired!",
    testFailed: "Trial Failed",
    wisdomBonus: "Bonus",
    xpGained: "XP",
    newRank: "New rank",
    nextGrade: "Next grade",
    maxLevelReached: "Maximum level reached!"
  },
  
  menu: {
    playButton: "PLAY",
    info: "Info",
    treasures: "Treasures",
    john: "John",
    reset: "Reset",
    restart: "🔄 Restart adventure?",
    confirmReset: "Are you sure you want to erase all your progress?",
    youWillLose: "You will lose:",
    wisdomPoints: "Wisdom Points",
    starsCollected: "stars collected",
    levelsUnlocked: "level(s) unlocked",
    spiritualRank: "Spiritual Rank",
    newAdventure: "🌟 New Adventure",
    readyToStart: "Ready to start?",
    startJourney: "Begin your journey into Scripture knowledge!",
    audioControls: "Audio controls",
    mute: "Mute sound",
    unmute: "Unmute sound",
    close: "Close",
    language: "Language",
    restartGame: "Restart game"
  },
  
  login: {
    title: "UNITY QUEST",
    subtitle: "Interactive biblical adventure",
    anonymousMode: "Play Anonymous",
    anonymousDesc: "Choose a username",
    emailMode: "Email Login",
    emailDesc: "Multi-device save",
    pseudo: "Username",
    pseudoPlaceholder: "Your username...",
    email: "Email Address",
    emailPlaceholder: "your@email.com",
    startPlaying: "Start playing",
    sendLink: "Send link",
    sending: "Sending...",
    connecting: "Connecting...",
    back: "Back",
    localSave: "Your progress will be saved locally",
    cloudSync: "No password required • Cloud sync",
    magicLink: "Receive a magic link by email",
    checkEmail: "📧 Check your email! A login link has been sent to you.",
    checkSpam: "Check your spam/junk folder too",
    waitingConnection: "Waiting for connection...",
    errorSend: "Error sending link",
    errorConnection: "Connection error",
    enterPseudo: "Please enter a username",
    enterEmail: "Please enter a valid email"
  },
  
  ranks: {
    seeker: "Seeker",
    disciple: "Disciple",
    servant: "Servant",
    witness: "Witness",
    guardian: "Guardian",
    wise: "Wise One",
    prophet: "Prophet",
    apostle: "Apostle",
    seekerDesc: "Beginning of the spiritual journey",
    discipleDesc: "Learning the teachings",
    servantDesc: "Service and devotion",
    witnessDesc: "Sharing the faith",
    guardianDesc: "Protection of the truth",
    wiseDesc: "Wisdom and knowledge",
    prophetDesc: "Spiritual vision",
    apostleDesc: "Spiritual master"
  },

  spiritualJourney: {
    beginning: "Beginning of spiritual journey",
    progress: "Spiritual progress",
    grade: "Grade",
    xp: "XP",
    level: "Level",
    nextGrade: "Next grade",
    still: "Still",
    maxLevelReached: "Maximum Level Reached!",
    masteredAllTeachings: "You have mastered all teachings",
    progression: "Progress"
  },
  
  treasures: {
    title: "📖 Bible Treasures",
    verse: "Verse of the day",
    fact: "Did you know?",
    treasure: "🎁 Hidden treasure",
    question: "Fun question",
    newTreasure: "New treasure",
    context: "Context",
    strongReference: "Biblical reference for in-depth study",
    johnGospelDescription: "The Gospel of John reveals Jesus' divinity through 21 inspiring chapters.",
    availableInLanguages: "Available in 14 languages",
    johnGospelTitle: "Gospel of John",
    johnGospelStats: "21 chapters • 878 verses • 14 languages",
    bibleReaderStrong: "Bible Reader + Strong",
    bibleReaderStrongDesc: "Complete reading with Strong's dictionary",
    johnTreasures: "John's Treasures",
    johnTreasuresDesc: "Biblical treasures and interesting facts",
    johnBibleReader: "John Bible Reader + Strong",
    emmanuelMemo: "Emmanuel's Memo",
    emmanuelMessage: "You have reached the mountain top, and knowledge is acquired. But unwavering faith requires life experience, mercy, and true love of Jesus. Life's various trials will test your path, the true way we choose, our heart. We remain imperfect, our desires are many, and love of gain or personal comfort too often becomes our selfish priority. Ask God for wisdom, His love, His strength, and humbly acknowledge your weaknesses, for we can hide nothing from Him... Nothing. No need to browse in private mode, use VPN, or even clear history. Fast like Daniel: deprive yourself for a moment of what you love most, in calm and wisdom. This will allow you to better hear God, to have a heart disposition ready for profound — and often painful — transformations. Then dreams and visions will become clearer, as with Joseph. But guard against pride: do not yield to 'I'm always right' or 'I know better.' For God's revelation is an extraordinary grace. Don't become a teacher of the Law, capable of crucifying Jesus again today, He who simply asks to love your neighbor... and grant them the same freedom you yourself receive. Having faith means entering into a relationship and a free journey of the heart. It will make you powerful heroes, capable of the impossible. For the experience of the supernatural, God's presence, His confirmation or blessing will make you truly unshakeable.",
    clarification: "Important clarification",
    bibleOffline: "Read Bible Offline",
    bibleStudy: "Bible study",
    version: "Version",
    openReader: "Open reader",
    features: "Features"
  },
  
  messages: {
    chooseCard: "🃏 Choose your power card",
    cardHelp: "It will accompany you for the 3 questions of this level",
    chooseAnswer: "📖 Choose your answer:",
    wrongAnswer: "❌ Wrong answer! You have {lives} life/lives left. Try again!",
    levelCompleted: "Level {level} Completed",
    questionsProgress: "Question {current} of {total}",
    starsCollected: "Stars collected: {current}/{total}",
    pointsEarned: "+{points} Wisdom Points",
    secondChance: "🔥 Your COURAGE card gives you a second chance! Try again with wisdom.",
    bonusApplied: "🃏 {card} Bonus: +{percent}% ({base} → {final})",
    defeatedQuote: "I can do all things in Him who strengthens me",
    perfect: "🃏 Perfect",
    good: "✅ Good",
    start: "✅ Start",
    secretLevel: "🔓 SECRET LEVEL UNLOCKED!",
    secretChapter: "🎁 Secret Chapter",
    returnToLevels: "Return",
    next: "Next",
    retry: "Retry",
    menu: "Menu",
    chapter: "Chapter",
    chapterStars: "Chapter stars",
    totalGlobal: "Global total",
    backToMenu: "Back to Menu"
  },
  
  chapters: {
    "1": "Genesis",
    "2": "Exodus",
    "3": "Jesus Christ",
    "4": "Crucifixion/Resurrection",
    "5": "Early Church",
    "6": "Paul's Missions",
    "7": "Letters/Revelation",
    "8": "Bonus Level"
  },
  
  cards: {
    faith: "FAITH",
    courage: "COURAGE",
    wisdom: "WISDOM",
    faithKnowledge: "+50% points",
    courageKnowledge: "Second chance",
    wisdomKnowledge: "Hint + bonus",
    faithDesc: "Maximum wisdom points obtained",
    courageDesc: "Normal points but second chance",
    wisdomDesc: "Hint revealed and +25% points",
    chooseCard: "🃏 Choose a knowledge gift",
    cardEffects: "🃏 Knowledge gift effects:",
    faithEffect: "+50% wisdom points - Maximum reward",
    courageEffect: "Normal points but second chance on error",
    wisdomEffect: "+25% points + hint revealed for each question",
    chooseWisely: "Choose wisely, as your choice will accompany you throughout the level"
  },
  
  challenge: {
    loading: "Loading level...",
    peace: "🕊️ May peace rest in your house",
    questionsAwaiting: "📚 3 knowledge questions await you",
    starsQuote: "Each star is a step toward divine light"
  },
  
  info: {
    title: "Unity Quest Chronicles of Peace",
    subtitle: "Interactive Scripture Discovery",
    yourStats: "Your Statistics",
    score: "Score",
    wisdom: "Wisdom",
    revelation: "Revelation",
    createdWith: "Created with ❤️",
    creator: "Emmanuel Payet",
    developerPassionate: "Developer passionate about spirituality",
    artistModule: "Emmanuel Artist",
    artistPortfolio: "Artistic portfolio and digital creations",
    features: "Features",
    multipleChapters: "91 levels across 8 biblical chapters",
    powerCards: "3 unique power cards (Faith, Courage, Wisdom)",
    progressionSystem: "Progression system with spiritual ranks",
    bibleTreasures: "Bible treasures and interesting facts",
    howToPlay: "How to Play",
    selectLevel: "Select an unlocked level",
    choosePowerCard: "Choose your power card",
    answerQuestions: "Answer the 3 knowledge questions",
    earnStars: "Earn stars based on remaining lives",
    tips: "Tips",
    readCarefully: "Read each question carefully",
    useWisdomCard: "Use the Wisdom card for hints",
    courageGivesSecondChance: "The Courage card gives you a second chance",
    faithMaximizes: "The Faith card maximizes your points",
    johnGospelTitle: "Gospel of John",
    johnGospelStats: "21 chapters • 878 verses • 14 languages",
    bibleReaderStrong: "Bible Reader + Strong",
    bibleReaderStrongDesc: "Complete reading with Strong's dictionary",
    johnTreasures: "John's Treasures",
    johnTreasuresDesc: "Biblical treasures and interesting facts",
    quickAccessChapters: "Quick access to famous chapters:",
    johnChapter1: "Prologue",
    johnChapter3: "Born Again",
    johnChapter14: "I Am the Way",
    johnChapter20: "Resurrection",
    version: "Version",
    madeWithLove: "Made with love for God's glory",
    backToMenu: "Back to Menu"
  },
  
  bible: {
    readerTitle: "📖 Bible Reader",
    johnReaderTitle: "Bible Reader - Gospel of John",
    john: "John",
    loading: "Loading Scriptures...",
    loadingChapter: "Chapter {chapter} of the Gospel of John. Full data is loading...",
    exploreWhileLoading: "Meanwhile, you can explore other chapters or use the search function.",
    menu: "Menu",
    tabBible: "Bible",
    tabUnity: "Unity",
    strong: "Strong",
    strongReferences: "Strong References",
    verses: "verses",
    previous: "Previous",
    next: "Next",
    strongDefinition: "Strong Reference",
    word: "Original word",
    transliteration: "Transliteration",
    close: "Close",
    share: "Share",
    shareVerse: "Share this verse",
    shareSelection: "Share selection",
    pronunciation: "Pronunciation",
    meaning: "Meaning",
    definition: "Complete definition",
    usage: "Biblical usage",
    etymology: "Etymology",
    search: "Search...",
    definitionNotFound: "Definition not found",
    definitionLoading: "Definition to load...",
    imageFeatureComing: "🖼️ Image feature coming!",
    copyError: "Copy error:",
    error: "❌ Error",
    errorLoading: "Loading error",
    retry: "Retry",
    readerInDevelopment: "John Bible Reader in development...",
    verseCopied: "Verse copied to clipboard!",
    moreReferences: "and {{count}} other references",
    copyright: "📖 Public Domain • Strong"
  },
  
  bibleResources: {
    inAppReader: {
      name: "Read in App",
      description: "Integrated Bible reader with Strong",
      features: {
        offline: "Offline",
        strong: "Strong Numbers",
        navigation: "Navigation"
      }
    }
  },
  
  mountain: {
    spiritualAscension: "Spiritual Ascension",
    levelProgress: "Level {{level}} • {{stars}} stars obtained",
    combo: "Combo",
    perfectMessage: '"For my thoughts are not your thoughts" - The lamb climbs toward perfect wisdom',
    goodMessage: '"Seek and you shall find" - The ascent continues toward truth',
    startMessage: '"Faith, even small as a mustard seed" - First step toward light',
    skipAnimation: "Skip animation",
    chapter: "Chapter",
    chapterProgress: "Chapter {{current}}/{{total}}",
    secret: "Secret",
    waypoints: {
      start: "Start",
      firstSlope: "First slope",
      lastSlope: "Last slope"
    },
    status: {
      defeat: "Defeat",
      inProgress: "In progress"
    }
  },
  
  quotes: {
    wisdom: "True knowledge leads to eternal wisdom",
    faith: "I can do all things through Christ who strengthens me",
    hope: "The Lord is my shepherd, I shall not want",
    love: "For God so loved the world...",
    perseverance: "Faith requires perseverance",
    defeat: "Failure invites growth in knowledge"
  },

  philosophy: {
    title: "Philosophical Dimension",
    loveOntology: "Love as ontological principle",
    loveDescription: "Jesus transforms love into a metaphysical foundation: loving God and others becomes the very structure of authentic being. Love transcends morality to become a mode of existence.",
    newCommandment: "The New Commandment",
    jesusEmphasizes: "When Jesus gives a new commandment, he emphasizes the verb to love",
    loveGod: "Love God with all your heart",
    loveNeighbor: "Love your neighbor as yourself",
    lovePriority: "Love becomes a priority"
  },

  unity: {
    title: "Unity That Produces Peace",
    author: "By Emmanuel",
    subtitle: "Each religion has its reasons, our borders come from the past",
    description: "Module of reflection on spiritual unity and peace between traditions",
    
    nav: {
      intro: "Intro",
      religions: "Religions",
      common: "Common Grounds",
      humility: "Humility"
    },

    intro: {
      emmanuelSays: "Emmanuel says:",
      ourBordersFromPast: "Our borders come from the past",
      bordersDescription: "Each religion has its reasons. Our divisions are not God's will, but the fruit of history, geography, culture.",
      divisionWork: "Division, isolation by barriers and borders of fear, is the work of our perilous past.",
      sharedHumanity: "We are born into different traditions, but we share the same humanity.",
      
      revelationTitle: "A revelation that opens eyes",
      surprisedLearning: "I was surprised one day to learn that Jews are also sons of God.",
      angelMichael: "That the angel Michael is a great messenger for Muslims.",
      jesusNames: "That Jesus, Isa, Yeshua are known in various ways, even in the Christian faith.",
      diverseTribes: "For there are various tribes, various churches, various families.",
      sameRealities: "We speak of the same realities with different names. This should bring us closer, not divide us.",
      
      respectTitle: "Respect: foundation of humility, guardian of peace",
      respectFoundation: "Even if brotherhood is not objectively applicable, respect is the foundation of humility, guardian of peace.",
      understandingProcess: "Understanding and respecting other religions is a process of humility. It doesn't mean renouncing your faith, but accepting that others also seek truth.",
      
      whatBindsUs: "What truly binds us",
      seekingRelation: "Every religion seeks relationship. Every person seeks peace.",
      loveOfGod: "This relationship that binds us all is God's love.",
      universalLink: "Love is the universal link that transcends all borders, all languages, all traditions.",
      
      inclusionTitle: "Inclusion: even vaster than religious diversity",
      vastInclusion: "Religious diversity is vast, but even vaster is the inclusion of others where they are called by God.",
      respectDifferences: "Respect for differences in color, genders, inclusion makes us unique humans, loved by God.",
      jesusCommand: "Jesus said: 'Love one another.'",
      
      communitySpirit: "Breaking with communitarian spirit",
      communityTerror: "Communitarianism is the disguised terror of hatred of evil. It transforms faith into tribal identity, into a wall that separates, into a weapon that hurts.",
      pridePolitics: "Our party spirits push us to the politics of pride:",
      prideQuotes: "I'm right to hurt you. I'm right period. I'm right to wage good war.",
      watchHeart: "Beware of the heart of the problem, the root of our own evil.",
      refuseDivision: "Let us refuse this drift that divides humanity in God's name.",
      
      freedomTitle: "To be free, make others free",
      freedomOthers: "To be free, we must know how to make others free too.",
      defendFreedom: "My freedom to believe has value only if I defend that of all. Oppressing others is condemning myself to servitude.",
      
      changeHeartTitle: "Change your heart: be a true lantern",
      recognizeDecency: "Recognize that decency is not the same for everyone.",
      changeYourself: "Change yourself first, progressively. Change your heart, transform your life, be a true lantern.",
      beExample: "Don't pee on the lawn, for it will perish... Don't impose your rules on others, but become a living example through your transformation.",
      
      childrenPeaceTitle: "Our children will live in peace in diversity",
      avoidPursuit: "Avoid pursuing word for word.",
      peaceChoice: "If we choose respect today, our future children will live in peace in diversity. Peace begins with us, now, in our words and actions.",
      
      discernTitle: "Discern: man or God?",
      manSpeaks: "Too often, it's man's heart that speaks and divides.",
      seeFruits: "We see it by the fruit of words and speech: theft, destruction, wars. Beware of word manipulation to compete for one's own pride.",
      
      lookPeaceTitle: "Look to peace and love",
      lookPeaceAll: "Look to peace for all, look to God's love for others.",
      godSpeaks: "When words produce peace, reconciliation, unity: it's God speaking. When they produce division, hatred, war: it's man's pride.",
      
      watchWordsTitle: "Watch our words",
      watchShares: "Watch our shares, watch our harmful words against others.",
      disqualify: "These evils disqualify us. The fruits of our words reveal who speaks in us: God or our own pride?",
      
      wisdomMaturityTitle: "Wisdom and maturity facing our nature",
      useScriptures: "Using scriptures (laws) to harm is our habit, an almost natural and mischievous process.",
      hateSin: "Facing our sinful nature, we abhor evil and people who do evil.",
      separateActs: "But through wisdom and maturity, we must separate the acts of our human poverty.",
      compassionNature: "Recognize our animal nature that needs compassion and grace, while protecting ourselves according to contexts.",
      
      denounceLeadersTitle: "Denounce corrupt spiritual leaders",
      watchLeaders: "Beware of spiritual leaders or rulers who seek to keep their drift and dishonesty secret.",
      noJustice: "Without justice, no example. Without knowledge, other victims perish through our silence.",
      silenceComplicity: "Silence in face of injustice is complicity. Truth must be told to protect others."
    },

    religions: {
      title: "Various Religions, One Humanity",
      intro: "Christians, Muslims, Jews, Hindus, Buddhists... and many others: all seek truth",
      theirGod: "Their God:",
      theGood: "The Good:",
      theEvil: "The Evil:",
      theAngels: "The Angels:",
      theUnity: "Unity:",
      values: "Values:",
      
      christianity: {
        name: "Christianity",
        godText: "One unique God, Creator Father",
        goodText: "Love of neighbor, forgiveness, peace",
        evilText: "Hatred, selfishness, violence",
        angelsText: "God's messengers (Gabriel, Michael...)",
        unityText: "One God for all people",
        values: "Love,Forgiveness,Peace,Humility,Justice"
      },
      islam: {
        name: "Islam",
        godText: "Allah, the Unique and Merciful",
        goodText: "Justice, charity, peace",
        evilText: "Oppression, lies, division",
        angelsText: "Angels of light (Jibril, Mikail...)",
        unityText: "There is no god but Allah",
        values: "Peace,Justice,Charity,Respect,Mercy"
      },
      judaism: {
        name: "Judaism",
        godText: "YHWH, the unique Eternal",
        goodText: "Justice, compassion, life",
        evilText: "Oppression, slander, hatred",
        angelsText: "Heavenly messengers (Malakhim)",
        unityText: "Hear O Israel, the Lord is One",
        values: "Justice,Life,Memory,Peace,Study"
      },
      hinduism: {
        name: "Hinduism",
        godText: "Brahman, the supreme reality",
        goodText: "Non-violence (ahimsa), righteous duty",
        evilText: "Violence, spiritual ignorance",
        angelsText: "Devas, benevolent celestial beings",
        unityText: "All is One in Brahman",
        values: "Non-violence,Truth,Compassion,Purity,Harmony"
      },
      buddhism: {
        name: "Buddhism",
        godText: "No creator god, but respect for sacred",
        goodText: "Compassion, wisdom, peace",
        evilText: "Suffering caused by ignorance",
        angelsText: "Bodhisattvas, enlightened beings",
        unityText: "Buddha nature in everyone",
        values: "Compassion,Wisdom,Peace,Non-violence,Awakening"
      }
    },

    common: {
      title: "Common Grounds",
      intro: "Good, Evil, Angels, One Unique God, Relationship, Inclusion: what unites humanity",
      
      goodTitle: "The Good",
      goodDescription: "All religions teach love, justice and compassion",
      goodExamples: "Love your neighbor as yourself|Do good without distinction|Protect the weak and oppressed|Speak truth with kindness|Share what you have",
      
      evilTitle: "The Evil",
      evilDescription: "All condemn violence, lies and oppression",
      evilExamples: "Kill without justice|Steal and exploit|Lie and slander|Despise and humiliate|Divide and hate",
      
      angelsTitle: "Angels and Spiritual Beings",
      angelsDescription: "Almost all recognize benevolent celestial messengers",
      angelsExamples: "Angel messengers of God|Spiritual guides|Beings of light|Celestial protectors|Bodhisattvas and Devas",
      
      godTitle: "One Unique God (or Supreme Reality)",
      godDescription: "The majority believe in a unique source, a creator or ultimate reality",
      godExamples: "Christianity, Islam, Judaism: One God only|Hinduism: Brahman, the supreme One|Buddhism: Universal Buddha nature|All seek ultimate truth|All aspire to unity and peace",
      
      relationTitle: "Relationship and Peace",
      relationDescription: "What truly unites all religions and all people",
      relationExamples: "Every religion seeks relationship with the divine|Every person seeks peace|This relationship that binds us all is God's love|Love is the universal link|Peace is our common aspiration",
      
      inclusionTitle: "Inclusion: Loved by God",
      inclusionDescription: "Religious diversity is vast, but inclusion is even vaster",
      inclusionExamples: "Inclusion of others where they are called by God|Respect for differences in color|Respect for differences in genders|Inclusion makes us unique humans, loved by God|Jesus said: 'Love one another'",
      
      finalQuote: "I was surprised one day to learn that Jews are also sons of God, that the angel Michael is a great messenger for Muslims, that Jesus, Isa, Yeshua are known in various ways even in the Christian faith. For there are various tribes, various churches, various families.",
      finalQuote2: "We speak of the same realities with different names.",
      quoteAuthor: "Emmanuel, Unity That Produces Peace"
    },

    humility: {
      title: "7 Steps Toward Humility and Peace",
      intro: "The process to overcome communitarianism and become a true lantern",
      
      step: "Step",
      action: "Action:",
      
      step1Title: "Recognize: our borders come from the past",
      step1Description: "Division, isolation by barriers and borders of fear, is the work of our perilous past. Our divisions are not God's will.",
      step1Action: "Accept that my tradition is only mine, and I am part of the diversity willed by God",
      
      step2Title: "Respect: foundation of humility, guardian of peace",
      step2Description: "Even if brotherhood is not objectively applicable, respect is the foundation.",
      step2Action: "Respect all beliefs, even without understanding everything",
      
      step3Title: "Understand without judging",
      step3Description: "Today, understanding others is a necessary process of humility.",
      step3Action: "Truly listen to what others believe, without judgment",
      
      step4Title: "Change yourself first: be a true lantern",
      step4Description: "Recognize that decency is not the same for everyone. Change your heart progressively, transform your life, become a true light.",
      step4Action: "Don't impose my rules on others, but be a living example through my transformation",
      
      step5Title: "Break with communitarian spirit",
      step5Description: "Our party spirits push us to the politics of pride: 'I'm right to hurt you, I'm right period.' Beware of the heart of the problem, the root of our own evil.",
      step5Action: "Refuse divisive discourse in my own community",
      
      step6Title: "To be free, make others free",
      step6Description: "My freedom depends on what I grant to others.",
      step6Action: "Defend everyone's freedom of conscience, not just mine",
      
      step7Title: "Avoid pursuing word for word",
      step7Description: "Don't respond to hatred with hatred, to insults with insults.",
      step7Action: "Choose peace so our children live in peace in diversity",
      
      discernFruits: "Discern by Fruits",
      humanHeart: "The heart of man that divides",
      lookGodLove: "Look to God's love",
      watchOurWords: "Watch Our Words",
      
      prideFruits: "Fruits of human pride",
      godFruits: "Fruits of God's love",
      
      finalQuote: "Even if brotherhood is not objectively applicable, respect is the foundation of humility, guardian of peace. Avoid pursuing word for word, and our future children will live in peace in diversity.",
      author: "Emmanuel"
    },

    footer: {
      quote: "Respect is the foundation of humility, guardian of peace. Our children will live in peace in diversity if we choose respect today.",
      author: "Emmanuel, Unity That Produces Peace"
    }
  },

  footer: {
    version: "Version 1.0 • 2024",
    dedication: "Developed for God's glory"
  },

  endCredits: {
    lines: [
      { text: "You have reached the mountain top", delay: 2000 },
      { text: "You have learned many things", delay: 2000 },
      { text: "Your faith grows every day", delay: 2000 },
      { text: "You experience beautiful things with God", delay: 2000 },
      { text: "Difficulties help you grow", delay: 2000 },
      { text: "They show what is important", delay: 2000 },
      { text: "Be humble like a little child", delay: 2000 },
      { text: "God loves your simple heart", delay: 2000 },
      { text: "In silence you hear God", delay: 2000 },
      { text: "Fasting frees your spirit", delay: 2000 },
      { text: "Pray and God will guide you", delay: 2000 },
      { text: "Pride hides the truth", delay: 2000 },
      { text: "God's grace is magnificent", delay: 2000 },
      { text: "It comes from God's heart", delay: 2000 },
      { text: "True faith sets you free", delay: 2000 },
      { text: "It unites you with God forever", delay: 2000 },
      { text: "Love can change everything", delay: 2000 },
      { text: "It can work miracles", delay: 2000 },
      { text: "Repeat with me", delay: 2000 },
      { text: "Jesus, I give you my heart", delay: 3000 },
      { text: "You are my king, my savior", delay: 3000 },
      { text: "Come transform my life", delay: 3000 },
      { text: "The journey continues now", delay: 2000 },
      { text: "Share this beautiful light", delay: 2000 },
      { text: "Toward life that never ends", delay: 2000 },
      { text: "Keep love in your heart", delay: 3000 },
      { text: "Like a precious treasure", delay: 3000 }
    ],
    finalMessages: {
      congratulations: "CONGRATULATIONS!",
      proud: "WE ARE PROUD OF YOU",
      courage: "VALIANT COURAGE",
      peace: "PEACEBEARER",
      child: "CHILD OF GOD",
      blessing: "MAY GOD BLESS YOU",
      continue: "Continue"
    }
  },

  validation: {
    pseudoTooShort: "Username must contain at least 2 characters",
    enterPseudo: "Please enter a username"
  },

  pseudoSetup: {
    title: "Choose your player username",
    profileConfig: "Profile Configuration",
    connectedWith: "Connected with",
    playerPseudo: "Player Username",
    pseudoPlaceholder: "Your username...",
    pseudoRequirements: "2-20 characters • Displayed in rankings",
    suggestions: "Suggestions",
    confirm: "Confirm Username",
    errorConfig: "Configuration error"
  },

  navigation: {
    back: "Back"
  },

  console: {
    starsUnlocked: "stars reached! Chapter unlock",
    defeatAnimation: "Defeat... J.C. descends"
  },

  errors: {
    progressionError: "Progression error",
    invalidProgressionData: "Invalid progression data:"
  },

  levels: {
    chaptersAndBonus: "8 chapters + bonus level",
    bonusUnlocked: "🎁 BONUS LEVEL UNLOCKED! You have reached spiritual perfection!",
    starsToDiscover: "273 {stars} to discover",
    starsPerLevel: "3 {stars} per level"
  },

  architecture: {
    title: "⚙️ Technical architecture",
    react: {
      name: "React",
      description: "Modern interface"
    },
    modules: {
      name: "Modules",
      description: "Dynamic loading"
    },
    localStorage: {
      name: "LocalStorage",
      description: "Local save"
    },
    tailwind: {
      name: "Tailwind",
      description: "Responsive design"
    }
  },

  gameManager: {
    title: "Game save",
    saveLocal: "Local save",
    saveCloud: "Cloud synchronized",
    saveWaiting: "Waiting for connection",
    lastSave: "Last save:",
    never: "Never",
    saving: "Saving...",
    save: "Save",
    loadProgress: "Load",
    loading: "Loading...",
    autoSaveEnabled: "💾 Auto-save enabled",
    saveError: "❌ Save error",
    gameSaved: "✅ Game saved!",
    gameLoaded: "✅ Game loaded!"
  }
};

export default uiTranslations;
